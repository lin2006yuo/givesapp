<template>
  <div class="pool">
    ...
    >
      ...
      >
        <item-card :pool-type="poolType" :card-info="item" :itemId="item.id"></item-card>
      </cell-box>
      <cell-box v-show="isOops">
        <oops :oop-type="poolType" class="pool-oops"></oops>
      </cell-box>
      <inline-loading v-show="this.page !== 1 && isLoading"></inline-loading>
      <divider v-show="itemList.data.length && !itemList.nextPageUrl" class="g-divider">沒有更多了</divider>
    </group>
    <side-bar @search="search" @hideSideBar="toggleShow" :pool-type="poolType" :is-show="isShow"></side-bar>
  </div>
</template>

<script>
import { XInput, XButton, CellBox, Divider } from "vux";
...
export default {
    ...
    itemList() {
      ...
      switch (this.poolType) {
        case "gift":
          ...
          break;
        case "need":
          itemList = this.needList;
          break;
        case "lovebox":
          itemList = this.loveboxs;
          break;
      }
      return itemList;
    },
  ...
  created() {
    ...
    if (
      roleMap[this.poolType] &&
      typeof roleMap[this.poolType][userRoleType] === "function"
    ) {
