      ...
      <div class="menu-content" v-show="isShow">
        <div class="content">
          <group :title="poolType === 'gift' ? '篩選器' : '會員類別'">
            <filter-option
              @click.native="selectOptions('other', index)"
              :selected="filterSelected.other[index]"
              :key="'other' + index"
              :option="option"
              v-for="(option, index) in (poolType === 'gift' ? filterOptions.isNew : filterOptions.userRoleType)"
            ></filter-option>
          </group>
          <group title="排序方式">
            <filter-option
              @click.native="selectOptions('sort', index)"
              :selected="filterSelected.sort[index]"
              :key="'sort' + index"
              :option="option"
              v-for="(option, index) in filterOptions.sort"
            ></filter-option>
          </group>
          <group title="物品類別">
            <filter-option
              @click.native="selectOptions('category', index)"
              :selected="filterSelected.category[index]"
              :key="'category' + index"
              :option="option"
              v-for="(option, index) in filterOptions.category"
            ></filter-option>
          </group>
        </div>
        <div class="btn-box">
...
import { Radio, Group, XButton, Flexbox, FlexboxItem } from "vux";
    ...
    selectOptions(type, idx) {
      ...
      switch (type) {
        case "other":
          options =
            this.poolType === "gift"
              ? this.filterOptions.isNew
              : this.filterOptions.userRoleType;
          break;
        case "sort":
      ...
      options.forEach((item, index) => {
        if (index !== idx) {
          ...
          this.$set(
            ...
            index,
            !this.filterSelected[type][index]
          );
          const isSelected = this.filterSelected[type][index];
          if (type === "other") {
            if (this.poolType === "gift") {
              this.searchType = isSelected
                ? this.filterOptions.isNew[index].key
                : "";
            } else {
              this.searchType = isSelected
                ? this.filterOptions.userRoleType[index].key
                : "";
            }
          } else if (type === "sort") {
            ...
            this.category = isSelected
              ? this.filterOptions.category[index].key
              : "";
          }
        }
      });
