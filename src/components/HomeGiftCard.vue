  ...
  <div class="hcard-container">
    <div class="hcard-userinfo">
      <router-link :to="userInfo.id === userId ? '/userinfo' : `/user/${userId}`">
        <span class="hcard-pics-wrapper">
          <img class="hcard-userinfo-pics" :src="smallAvatar || UserDefault" alt="用戶頭像">
        </span>
      </router-link>
      <div class="hcard-username-wrapper">
        <router-link
          :to="userInfo.id === userId ? '/userinfo' : `/user/${userId}`"
          class="hcard-username"
        >{{userName}}</router-link>
        <span class="hcard-address" v-if="city && district">
          <img class="hcard-address-icon" :src="LocationIcon" alt>
          {{city}}, {{district}}
        </span>
      </div>
    ...
    <div class="hcard-action">
      ...
      <div v-if="userInfo.id !== userId" class="hcard-action-right">
        ...
        >{{isTraced ? (isTracing ? '已追蹤' : '追蹤') : (isTracing ? '取消中...' : '追蹤中...')}}</button>
        <button
...
export default {
  ...
  methods: {
    handleToggleTrace(giftId, isTracing) {
      ...
      if (this.isTraced) {
        this.isTraced = false;
        this.$store.dispatch("toggleTraceGift", {
          giftId,
          isTracing,
          cb: this.handleToggleTraceCb
        });
      }
    },
    handleToggleTraceCb() {
    ...
    handleRequest(id) {
      ...
      if (!memberData.isValidBaseInfo(this.$store.state)) {
        ...
        setTimeout(() => {
          this.$router.push(
            `/user/${this.userInfo.id}/editBaseInfo?redirect=/request/${id}`
          );
        }, 3000);
        ...
        setTimeout(() => {
          this.$router.push(
            `/user/${this.userInfo.id}/editAdr?redirect=/request/${id}`
          );
        }, 3000);
      } else {
