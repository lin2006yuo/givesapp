...
export default {
  ...
  methods: {
    ...
    loadMoreEnvMsgCb (obj) {
      ...
      this.updateEnvMessageList(newItems)
      this.updateEnvMessageNextPageUrl(nexPageUrl)
      if (nexPageUrl) {
        const page = this.envMessage.page + 1
    ...
    loadMoreSysMsgCb (obj) {
      ...
      let {data, next_page_url: nexPageUrl} = obj
      data = data.map((v, i) => {
      ...
      this.updateSysMessageList(data)
      this.updateSysMessageNextPageUrl(nexPageUrl)
      if (nexPageUrl) {
        const page = this.sysMessage.page + 1
