    ...
    <div class="pics">
      <swiper
        ...
        <swiper-item
          ...
          height="500px"
          :key="index">
          <p class="img-item" :style="{backgroundImage: `url(${smartImagePath(item, false)})`}"></p>
        </swiper-item>
      </swiper>
    ...
    <div class="tabs">
      ...
      <div class="content">
        <div v-show="tabType === key" :key="value.key" v-for="(value, key) in tabItems" :class="value.type">
          ...
          <comment-item
            v-if="!(!value.data.length && !value.isLoading) && key === 'thank'"
            :item-type="itemType"
            :c-type="value.type"
            :key="'parent' + index"
            :info="item"
          ...
          <love-box-request
            v-if="!(!value.data.length && !value.isLoading) && key === 'request'"
            :key="index"
            :info="item"
...
<script>
import { Tab, TabItem, Group, XInput, XButton, XTextarea, Swiper, SwiperItem, Tabbar,
  TabbarItem, InlineLoading, XNumber } from 'vux'
...
export default {
  ...
  methods: {
    ...
    addToShopCart () {
      ...
      if (this.userInfo.catHash !== 'commonweal' &&
        ...
        if (this.passDays <= 4) {
          return this.$vux.toast.text('上架5天內，公益團體/學校單位優先索取', 'bottom')
        }
      ...
      if (!this.isAdd) {
        ...
        this.addlovboxItem({
          ...
          cb: (data) => {
            this.isAdd = false
            if (data.error) {
              return this.$vux.toast.text('加入禮物車失敗，請稍後重試', 'bottom')
            }
            this.getLoveCart()
            this.getLoveboxInfo({
              id,
  ...
  computed: {
    ...
    passDays () {
      const days = new Elapsed(new Date(dateStandard(this.currentLovebox.on_shelve_at)), new Date()).days.num
