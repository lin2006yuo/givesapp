  ...
  <div class="g-block null">
    ...
    <div class="pad-search-bar">
      <button class="btn-to-search" @click="toSearch">
        <img class="icon" src="../assets/icon/gift/icon-search.png">
        搜尋您心儀的禮物或者需求
      </button>
      <img @click="toFeedback" :src="iconNav" class="right-icon" width="22" height="22">
    </div>
    <div class="home-container">
      <swiper loop dots-class="home-container-dots" :show-desc-mask="false">
        <swiper-item :key="index" v-for="(item, index) in bannerList">
          <a
            ...
            target="_blank"
          >
            <img class="home-swiperitem-img" :src="item.img | smallImage" alt>
            <span v-if="item.title" class="home-swiperitem-desc">{{item.title}}</span>
          </a>
        </swiper-item>
      </swiper>
      <home-modules></home-modules>
      <section v-if="commonwealList.length" class="home-commonweal-panel">
        <give-title title="公益團體需求" link="/needs?userRoleType=commonweal"></give-title>
      ...
      <section v-if="giftList.length" class="home-cabinet-panel">
        <router-link to="/cabinetGift" class="home-cabinet-link">
          <give-title moreText="掌櫃快捷寄送" title="名櫃好禮" link="/cabinetGift"></give-title>
      ...
      <div class="home-loading" v-if="!giftsList.length">
        <spinner type="lines"></spinner>載入中...
      </div>
      <home-gift-card
        v-else
